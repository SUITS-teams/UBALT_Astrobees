...
export function readdirPromised(path) {
    return new Promise((resolve, reject) => fsRaw.scandir(new Fs(), path, 0, (error, req) => {
        if (error) {
            reject(error);
            return;
        }
...
export class Stats {
    ...
    isBlockDevice() { return this.type === "block"; }
    isCharacterDevice() { return this.type === "char"; }
    isDirectory() { return this.type === "dir"; }
    isFIFO() { return this.type === "fifo"; }
    isFile() { return this.type === "file"; }
    isSocket() { return this.type === "socket"; }
    isSymbolicLink() { return this.type === "link"; }
}
const statCallback = (cb) => (err, val) => err ? cb(err) : cb(err, new Stats(val));
...
function realpathPromised(path) {
