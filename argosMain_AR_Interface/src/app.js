import { ImmersiveApp, ui, PrismType, input, color, HandGestureFlags,
  GestureInputEventData } from 'lumin';

import { mat4, vec3 } from 'gl-matrix';
import { menuButton } from './Components/uiComponents/ButtonArgos';
import { hoverText } from './Components/uiComponents/TextArgos';
import { mainMenuButtons } from './Components/uiComponents/globalVariables';
import { initScienceSamplingView, setStep } from './Components/systemComponents/scienceSampling/scienceSampling';
import { initTelemetry } from './Components/systemComponents/telemetry/telemetry';
// Test socket i o server
import io from 'socket.io-client/dist/socket.io';

var mainPrism;
var fingerTipPosition = vec3.create();
...
export class App extends ImmersiveApp {
  ...
  updateLoop (delta) {
    if (this.leftHand.gesture !== input.GestureType.NONE) {
      ...
      if (this.leftHand.timer <= 0.0) {
        // this.resolveLeftHandGesture(input.GestureType.NONE, 1.0, null);
      } else {
    ...
    if (this.rightHand.gesture !== input.GestureType.NONE) {
      ...
      if (this.rightHand.timer <= 0.0) {
        // this.resolveRightHandGesture(input.GestureType.NONE, 1.0, null);
      } else {
  ...
  getLocalFingerTipPosition () {
    ...
    fingerTipPosition = inverse * vec3.transformMat4(fingerTipPosition);
  ...
  triggerClickState (name) {
    switch (name) {
      case 'scienceBtn':
        ...
        break;
      case 'navBtn':
        // Call the function for nav button clicked.
        break;
      case 'toolBtn':
        // Call the function for tool button clicked.
        break;
      case 'roverBtn':
        // Call the function for rover button clicked.
        break;
    }
  }
  ...
  resolveRightHandGesture (gestureType, confidence, event) {
    ...
    if (gestureType !== input.GestureType.NONE && event !== null) {
      ...
      if (gestureType === input.GestureType.HAND_FINGER) {
        ...
        let localFingerTipPos = vec3.transformMat4([], fingerTipPosition, inverse);
